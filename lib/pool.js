...
setInterval(function() {
    var now = Date.now();
    var timeout = config.poolServer.connectionTimeout * 1000;
    var clients = stratumServer.getStratumClients();
    for(var minerId in clients) {
        var miner = clients[minerId];
        if(!miner.socket.writable) {
            if(now - miner.lastActivity > timeout) {
                stratumServer.removeStratumClientBySubId(minerId)
...
function removeConnectedWorker(client, reason) {
    ...
    redisClient.hincrby(redisPrefix + ":active_connections", login_parts.address_pid + "~" + login_parts.worker_id, -1, function(error, connectedWorkers) {
        if (reason === "banned") {
            ...
            if (!connectedWorkers || connectedWorkers <= 0) {
