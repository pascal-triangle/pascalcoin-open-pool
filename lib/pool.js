...
var waitForFirstJob = setInterval(() => {
    ...
    stratumServer = new stratum.Server(function(ip, port, login, callback) {
	    ...
	    // Record new miner in Redis
	    redisClient.hincrby(redisPrefix + ":ports:" + port, "users", 1)
    ...
    stratumServer.on("log", function(severity, message) {
	log(severity, logSystem, message);
    });
