...
var StratumClient = function(options) {
    ...
    function handleAuthorize(message, replyToSocket) {
        ...
        _this.port = options.socket.localPort;
    ...
    function sendJson() {
        if(!options.socket.writable) return;
        var response = "";
    ...
    this.manuallyAuthClient = function (username) {
        handleAuthorize({id: 1, params: [username]}, false /*do not reply to miner*/);
    };
...
StratumClient.prototype.__proto__ = EventEmitter.prototype;
 ...
 **/
var StratumServer = exports.Server = function StratumServer(authorizeFn) {

    //private members

    //ports, connectionTimeout, jobRebroadcastTimeout, banning, haproxy, authorizeFn

    var poolCfg = config.poolServer;
    ...
    var bannedIPs = {};
    var bannedMS = poolCfg.banning ? poolCfg.banning.time * 1000 : null;

    function checkBan(client) {
        if(poolCfg.banning && poolCfg.banning.enabled && client.remoteAddress in bannedIPs) {
    ...
    this.handleNewClient = function(socket) {
        ...
        var client = new StratumClient({
            ...
            socket: socket,
            banning: poolCfg.banning,
            connectionTimeout: poolCfg.connectionTimeout,
            tcpProxyProtocol: poolCfg.tcpProxyProtocol
        });
    ...
    this.broadcastMiningJobs = function(jobParams) {
        ...
        }, poolCfg.jobRebroadcastTimeout * 1000);
    };

    // use a short timeout so the code calling this
    // constructor has time to set up log event listener
    setTimeout(function () {

        setInterval(function() {
            // Look through bannedIPs for old bans and remove them in order to prevent a memory leak
	...
	_this.emit("log", "info", "Clear values for connected workers in redis database.");
	redisClient.del(redisPrefix + ':active_connections');

        var serversStarted = 0;
	config.poolServer.ports.forEach(function(portCfg) {
	    if(portCfg.port && portCfg.diff) {
		_this.emit("log", "info", "Clear values for port %s in redis database.", [portCfg.port]);
		redisClient.del(redisPrefix + ':ports:' + portCfg.port);
		redisClient.hset(redisPrefix + ':ports:' + portCfg.port, 'port', portCfg.port);
		net.createServer({allowHalfOpen: false}, function(socket) {
                    _this.handleNewClient(socket);
		}).listen(parseInt(portCfg.port), function() {
                    if(++serversStarted == Object.keys(config.poolServer.ports).length)
			_this.emit("started");
		});
	    } else {
		_this.emit("log", "error", "Port %s has invalid configuration", [portCfg.port]);
	    }
        });
    ...
    this.manuallyAddStratumClient = function(clientObj) {
        ...
        if(subId != null) { // not banned!
            stratumClients[subId].manuallyAuthClient(clientObj.workerName);
            stratumClients[subId].manuallySetValues(clientObj);
        }
    };

};
StratumServer.prototype.__proto__ = EventEmitter.prototype;
