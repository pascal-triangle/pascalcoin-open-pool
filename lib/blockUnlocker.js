...
function getShares(blocks, callback) {
    ...
    redisClient.multi(redisCommands).exec(function(error, replies) {
        ...
        for (var i = 0; i < replies.length; i++) {
            ...
            //reward -= 6; // reserve 0.0006 PASC for sweeping balance to main PASA and PASA payouts
            // we don't need to reserve any molinas since we will only do one op per account per block
...
function handleUnlocked(blocks, callback) {
    ...
    blocks.forEach(function(block) {
        ...
        Object.keys(block.workerRewards).forEach(function(worker) {
            ...
            log("info", logSystem, "Block %d payment to %s for %d%% of total block score: %d",
                [block.height, worker, block.workerRewards[worker][1]*100,
                 (block.workerRewards[worker][0]/10000).toFixed(4)]);
        });
