            ...
            if(err) {
                ...
                if(devFee > 1) {
                    daemonRpc.async("sendto", {
                        ...
                        if(data.hasOwnProperty("error")) {
                            log("error", logSystem, "Error making devfee payment: %s PASC", [(devFee / 10000).toFixed(4)]);
                        } else {
                            redisClient.hset(redisPrefix + ":stats", "devFee", 0);
                            redisClient.hincrby(redisPrefix + ":stats", "devFeePaid", devFee);
                            log("info", logSystem, "Devfee payment made: %s PASC", [(devFee / 10000).toFixed(4)]);
                        }
                    });
                }
            }
