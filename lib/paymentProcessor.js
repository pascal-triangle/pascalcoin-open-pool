...
function makePayment(accounts, keys, balances, pasaBalances, minPayoutLevel, publicKeys, callback) {
    ...
    async.eachLimit(pasaPayments, 10, function(worker, callback2) {
        if(publicKeys[worker] == false) {
        ...
        daemonRpc.async("changekey", {
            ...
            if(data.hasOwnProperty("error")) {
                ...
                log("info", logSystem, "Sent PASA %s to %s", [data.result.account, worker]);

                var redisCommands = [
                    ["hincrby", redisPrefix + ":workers:" + worker, "pasaBalance", "-" + Math.floor(config.payments.pasaThreshold * 10000)],
                    ["hincrby", redisPrefix + ":workers:" + worker, "pasaPaid", 1],
                ];
