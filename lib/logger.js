...
global.log = function(severity, system, text, data) {
	...
	if(config.logging.console.colors) {
            if(/Block (found|accepted)/.test(text)) {
                // small hack to make these messages green
	        console.log(clc.green(time) + clc.white.bold(" [" + system + "] ") + clc.green.bold(formattedMessage));
            } else {
