...
function collectStats() {
    ...
    async.parallel({
        ...
        devfee: function(callback) {
            redisClient.hmget(redisPrefix + ":stats", ["devFee", "devFeePaid"], function(error, replies) {
                callback(error, {
                    owed: replies[0] ? replies[0] : 0,
                    paid: replies[1] ? replies[1] : 0,
                    fee: config.fees.devFee ? config.fees.devFee : 0.5
                });
            });
        },
    ...
    async.waterfall([
        ...
        function(callback) {
            redisClient.multi([
                ...
                ["zrange", redisPrefix + ":blocks:candidates", 0, -1],
                ["hget", redisPrefix + ":stats", "devFee"],
                ["hget", redisPrefix + ":stats", "devFeePaid"]
            ]).exec(function(error, replies) {
                if(error) {
                ...
                callback(null, replies[0], replies[1], replies[2], replies[3]?replies[3]:0, replies[4]?replies[4]:0);
            });
        },

        //Get worker balances
        function(workerKeys, blocks, candidates, devFee, devFeePaid, callback) {
            var redisCommands = workerKeys.map(function(k) {
                return ["hmget", k, "lockedBalance", "balance", "paid"];
            });
            redisClient.multi(redisCommands).exec(function(error, replies) {
                if(error) {
                ...
                callback(null, replies, blocks, candidates, devFee, devFeePaid);
            });
        },
        function(workerData, blocks, candidates, devFee, devFeePaid, callback) {
            var stats = {
                totalOwed: parseInt(devFee),
                totalPaid: parseInt(devFeePaid),
                totalRevenue: 0,
                totalDiff: 0,
                totalShares: 0,
                blocksOrphaned: 0,
            ...
            for(var i = 0; i < workerData.length; i++) {
                ...
                stats.totalOwed += parseInt(workerData[i][1]) || 0;
                stats.totalPaid += parseInt(workerData[i][2]) || 0;
                stats.totalWorkers++;
            }
            ...
            for(var i = 0; i < candidates.length; i++) {
                ...
                if(block.length > 5) {
                    if(block[6] == 1) {
                        ...
                        stats.totalRevenue += parseInt(block[5]);
                        stats.totalOwed += parseInt(block[5] * devFee / 100);
                    }
                }
            }
            callback(null, stats);
