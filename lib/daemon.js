...
var DaemonMiner = exports.Miner = function() {
    ...
    function submitBlock(block) {
        if(submittedBlocks.includes(block.height)) {
    ...
    client.on("close", () => {
	log("info", logSystem, "Connection closed!");
    });

    client.on("error", (err) => {
	log("info", logSystem, "Connection error %s", [err]);
    });

    log("info", logSystem, "Interface starting...");
    client.connect(port, host, () => {
	log("info", logSystem, "Interface connected!");
    });
